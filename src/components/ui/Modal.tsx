...
export function Modal({ open, title, onClose, children, footer, size = "md" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className={`relative z-10 flex max-h-[92vh] w-full flex-col rounded-xl ify-modal-panel shadow-xl ${size === "xl" ? "max-w-4xl" : size === "lg" ? "max-w-2xl" : "max-w-lg"}`}>
        ...
        {footer && <div className="flex shrink-0 justify-end gap-2 border-t border-[var(--border-light)] px-5 py-4">{footer}</div>}
      </div>
    </div>
  );
}
